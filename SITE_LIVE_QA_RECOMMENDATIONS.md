...
- Latest activation-checkpoint commits: frontend `e68f050` (`Add edge voice activation profile`) and backend `6f13d97` (`Add edge voice activation audit profile`). They add the `edge-voice` profile for tasks 1-3 only: Cloudflare KV/D1/rate/analytics, persistence, and voice/TTS, while keeping managed RAG on the current `hash`/`chroma` runtime. Frontend CI run `29760491606`, Cloudflare Pages, backend CI run `29760491913`, and the Railway deployment status for `6f13d97` all passed.
...
- Release audit activation profiles are source-ready for future rollout proof: `current`, `managed-rag`, `voice`, `persistence`, `edge-voice`, and `full-activation`. A public-only `--activation-profile edge-voice` check now blocks on the expected inactive tasks 1-3 pieces: frontend `voiceEnabled`, frontend `persistenceEnabled`, and backend `tts.status=ok`, while backend RAG remains expected on `hash`/`chroma`. A public-only `--activation-profile full-activation` still additionally expects managed RAG through OpenAI/Pinecone.
...
- Edge voice activation-checkpoint QA passed for the source tooling. Frontend `e68f050` passed `node --check scripts/activation-readiness.mjs`, `npm run qa:activation -- --profile edge-voice --plan`, `npm run qa:activation -- --profile edge-voice --runtime-config-json`, hosted CI run `29760491606`, and Cloudflare Pages. Backend `6f13d97` passed `py_compile`, focused release-audit tests (`11 passed`), hosted CI run `29760491913`, and Railway deployment status. The live `edge-voice` checks intentionally remain blocked until Cloudflare and Railway control-plane activation is completed: analytics KV is unbound, D1 is unbound, runtime `voiceEnabled` and `persistenceEnabled` are false, same-origin TTS fails closed with `tts_disabled`, and backend TTS is `unconfigured`.
...
- Frontend CI app-runtime Node 24 update:
  ...
  - CI Playwright server-ownership fix commit `84e01ce` is included in current frontend `main`. It removes the workflow-managed background preview server, relies on Playwright `webServer`, switches the mobile project to `Mobile Chrome`, keeps HTML artifacts for CI failures, and scopes brittle chat button locators inside the dialog.
- Frontend Wrangler pin update:
  - Frontend commit `76b97ba` adds exact devDependency `wrangler@4.112.0`, switches hosted CI and guarded `deploy.sh` to `./node_modules/.bin/wrangler`, and adds a deploy-helper preflight that asks operators to run `npm ci` if the pinned binary is absent.
  - Local Node 24 QA passed on 2026-07-20 with `node v24.18.0`, `npm ci`, `bash -n deploy.sh`, guarded non-deploy smoke, `./node_modules/.bin/wrangler --version` returning `4.112.0`, `npm run type-check`, `npm run lint`, `npm run build`, `./node_modules/.bin/wrangler pages functions build`, and full Playwright with one worker (`156 passed`). The package-lock comparison found zero existing package version changes and only Wrangler's dependency tree added.
  - Cloudflare Pages deployment succeeded; live `/build-manifest.json` returned commit `76b97ba`, unchanged code-bearing assets `/assets/index-Cld5-OrE.js`, `/assets/index-DH0EGGDC.css`, `/assets/StratumChat-5iN0axbq.js`, 13 total assets, and `Cache-Control: public, max-age=60, must-revalidate`. Live `/api/config` remains `ragEnabled: true`, `voiceEnabled: false`, `persistenceEnabled: false`; live same-origin `/api/health` and direct Railway `/api/health` returned healthy RAG and unconfigured TTS status; direct Railway `/api/runtime` returned `graph_runtime: "langgraph"`, `embedding_provider: "hash"`, `vector_store_provider: "chroma"`, and `llm_provider: "writer"`. Live `/api/tts` returns `503 tts_disabled`, and live `/api/analytics` returns `503 analytics_not_configured`.
  - Hosted GitHub Actions run `29742456851` did not start any steps because of a GitHub account billing/spending-limit failure, so hosted runner proof for `76b97ba` is pending.
- Frontend production live-smoke command update:
  - Frontend commit `bb8f3b4` adds `npm run qa:live`, a cache-busted and optional `EXPECTED_MANIFEST_COMMIT` assertion for `/build-manifest.json`, forbidden-copy scans on root HTML plus the current STRATUM chat asset, same-origin `/api/config`, `/api/health`, `/api/tts`, `/api/analytics` checks, and direct Railway `/api/health` plus `/api/runtime` checks.
  - Local QA passed on 2026-07-20 with `npm run lint`, Node 24 `npm run type-check`, `npm run build`, `./node_modules/.bin/wrangler pages functions build`, Node 24 `EXPECTED_MANIFEST_COMMIT=7bf0df6 npm run qa:live`, and post-deploy `EXPECTED_MANIFEST_COMMIT=bb8f3b4 npm run qa:live`.
  - Cloudflare Pages deployment succeeded; live smoke verified manifest commit `bb8f3b4`, unchanged code-bearing assets `/assets/index-Cld5-OrE.js`, `/assets/index-DH0EGGDC.css`, `/assets/StratumChat-5iN0axbq.js`, `ragEnabled: true`, `voiceEnabled: false`, `persistenceEnabled: false`, same-origin and direct Railway healthy RAG, `tts_disabled` no-store, `analytics_not_configured` no-store, and direct runtime providers `hash` / `chroma` / `writer`.
  - Hosted GitHub Actions run `29743225634` did not start any steps because of the GitHub billing/spending-limit blocker.
- Backend production live-smoke command update:
  - Backend commit `5793eee` adds `scripts/live_backend_smoke.py` and README usage for a safe deployed backend smoke. It verifies public `/api/health`, production CORS, `/api/runtime`, one grounded RAG SSE stream with citations, and one `X-Stratum-Eval: true` suppressed escalation SSE stream.
...
- Cross-surface release/governance audit update:
  - Backend commit `d45f4c9` adds `scripts/live_release_audit.py` and README usage for a non-mutating release audit. It checks GitHub `main` branch protection, required status/check state, Cloudflare Pages deployment state, the live frontend manifest and runtime flags, Railway deployment status, and backend health/runtime without printing secrets or mutating any control plane.
  - Local QA passed on 2026-07-20: `python -m py_compile scripts/live_release_audit.py`, public-only audit (`--skip-github`) with zero blockers, full audit exiting nonzero with four expected governance blockers, full pytest (`129 passed, 1 skipped`), Railway deployment success for `d45f4c9`, and post-deploy backend live smoke.
  - Full audit after deploy confirmed public runtime health and intentionally blocked on frontend branch protection, backend branch protection, frontend Actions billing failure, and backend Actions billing failure. Hosted backend CI run `29745225484` did not start any steps because of the GitHub billing/spending-limit blocker.
- Backend SOT QA hardening update:
  - Backend commit `406089f` hardens the graph SSE no-key fallback so an empty provider stream cannot checkpoint or return only the grounding preamble, keeps confirmed handoff copy name-neutral, removes duplicated local conversation-matrix fixtures in favor of the shared scenario source, adds answer-substance and citation-support checks to the deployed Phase 4 matrix, and lets `scripts/live_release_audit.py --include-conversation-matrix` run that matrix as an opt-in gate.
  - Local QA passed on 2026-07-20: `python -m py_compile` for touched backend modules/scripts/tests, focused SSE and conversation-matrix tests (`99 passed, 1 skipped`), RAG eval (`passed: true`, recall@10 `1.0`, groundedness proxy `1.0`, substantive no-key response), full pytest (`131 passed, 1 skipped`), and `git diff --check`.
  - Live QA passed after Railway deployed `406089f`: backend smoke passed; deployed conversation matrix passed with `54` scenarios, contract/expected/persona/no-hallucination/answer-substance rates all `1.0`, scripted escalation rate `0.2037`, and first-token p95 `126.92ms`; full release audit confirmed public runtime plus matrix health and intentionally blocked only on branch protection and GitHub Actions billing failures. Hosted backend CI run `29746454396` did not start any steps because of the GitHub billing/spending-limit blocker.
- Backend configurable release-audit expectations update:
  - Backend commit `ac6a69a` makes release-audit runtime expectations configurable through non-secret CLI flags and env vars while keeping the current production defaults at `ragEnabled=true`, `voiceEnabled=false`, `persistenceEnabled=false`, `graph_runtime=langgraph`, `session_store_backend=postgres`, `embedding_provider=hash`, `vector_store_provider=chroma`, `llm_provider=writer`, and `tts.status=unconfigured`.
  - Local QA passed on 2026-07-20: `python -m py_compile scripts/live_release_audit.py tests/test_live_release_audit.py`, focused release-audit tests (`5 passed`), full pytest (`136 passed, 1 skipped`), `git diff --check`, and `.venv/bin/python scripts/live_release_audit.py --skip-github` with zero blockers.
  - Live QA passed after Railway deployed `ac6a69a`: backend smoke passed; release audit with `--include-conversation-matrix` confirmed frontend commit `b51a623`, backend commit `ac6a69a`, frontend flags including `maxIntakeQuestions: 7`, backend runtime `hash` / `chroma` / `writer`, TTS `unconfigured`, and a deployed 54-scenario matrix with contract/expected/persona/no-hallucination/answer-substance rates all `1.0`, scripted escalation rate `0.2037`, and first-token p95 `103.32ms`. The audit still intentionally blocks only on branch protection and GitHub Actions billing failures.
- Backend max-intake release-audit gate update:
  - Backend commit `178124e` makes `scripts/live_release_audit.py` explicitly verify frontend `/api/config.maxIntakeQuestions`, defaulting to the seven-question SOT contract with non-secret CLI/env overrides for future planned intake changes.
  - Local QA passed on 2026-07-20: `python -m py_compile scripts/live_release_audit.py tests/test_live_release_audit.py`, focused release-audit tests (`7 passed`), full pytest (`138 passed, 1 skipped`), `git diff --check`, and `.venv/bin/python scripts/live_release_audit.py --skip-github` with zero blockers.
  - Live QA passed after Railway deployed `178124e`: backend smoke passed; release audit with `--include-conversation-matrix` confirmed frontend commit `0d9a142`, backend commit `178124e`, `maxIntakeQuestions: 7`, backend runtime `hash` / `chroma` / `writer`, TTS `unconfigured`, and a deployed 54-scenario matrix with contract/expected/persona/no-hallucination/answer-substance rates all `1.0`, scripted escalation rate `0.2037`, and first-token p95 `91.65ms`. The audit still intentionally blocks only on branch protection and GitHub Actions billing failures.
...
- Frontend commit `52cdf47` adds the source-controlled `npm run qa:live:rendered` production browser smoke. Frontend commit `b51a623` is the latest live-proofed asset hash refresh after the Pages fallback fix. Use rendered smoke after `qa:live` when frontend work needs production browser proof.
